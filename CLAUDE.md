...
- **`fb-md-editor` stays out of the core kit** — optional add-on module (guide §2 recommendation).
  Shipped 2026-08-28 as its own repo: [`sac-md-editor`](https://github.com/SACRVM/sac-md-editor).
...
- **Every release notifies all consumer repos: signed off 2026-08-23.** A kit release reaches a consumer only when it re-vendors (until the Tier-4 self-contained embed exists, a hosted app runs on the host's kit, so a new version lands only when the repo pulls it). So cutting a release ENDS by sending a re-vendor "update now" message — to every consumer app repo for a MINOR/MAJOR; **for small things (a reported bug fix, a minor tweak) only to the repos that reported it plus the hosts/suites (`sacrvm-desktop`, `umbrella-toolkit`)**, which roll it out to everything they host (amended 2026-09-25: don't make the user walk 12 repos for almost nothing). The `/release` command carries the step and the current consumer list. (The Tier-4 embed — a CI inliner that ships one self-contained `.js` with `sac-*` tags rewritten to a versioned prefix — is the decided-but-unbuilt endpoint that would remove the need; see the "Tier 4 vision" knowledge doc.)

- **Mobile / responsive foundation (2.6.0): signed off 2026-09-24.** Breakpoints are fixed values, not tokens: **`compact` ≤ 768px** (page layout) plus **`narrow` ≤ 480px** only where a component needs it. Page layout (`.main-layout`, rail, nav) reacts to `@media`; components react to their own container (`container-type: inline-size` + `@container`), because they also live in narrow windows and split panels. On compact: the rail becomes an off-canvas drawer opened by the `sac-nav` burger (one burger, not two); `sac-split` shows one panel at a time with a back affordance; `sac-dialog` becomes a bottom sheet. Desktop (≥1024px) must stay exactly as it was in 2.5.0.
  **The burger belongs to the app** (same day): the kit ships neutral building blocks with today's behaviour as the default — `rail`, `host-nav="always|wide"`, `sections-nav="always|wide"` (drop sections that the rail already lists) (hide the suite list on compact when a dashboard is the phone's main level), a `panel` slot for any app content, `open()/close()/toggle()` and `sac:nav-open/close`. "Dashboard is the main navigation level on phones" is a documented best practice, not a kit rule. No collapsible header row inside the drawer: when an app has sections AND a rail they stack.
  **Phone ribbon has two faces** (same day): menu closed = burger + app name (text) + the app's own controls — no icon row (bare glyphs read as cryptic). Menu open = the desktop title bar: controls slide out to the right while ⌂ + the suite's name (truncates first) + app icon ease in before the app name — exactly the desktop brand row (colours, sizes, spacing); the app name keeps its desktop colour closed and open, so nothing changes colour; one smooth transition, no swap (where you are + the way home); no head row inside the drawer. `compact-title` overrides the closed-face name (e.g. when app-name is a version).

- **Global runtime language (i18n): signed off 2026-09-24, built in 2.12.0.** Language switching is kit work, not app work (Pixel Atelier dropped its own DE/EN toggle for it). Modelled on the theme: ONE global value, owned by the host on a desktop, a `<sac-lang-toggle>` in the app's own nav when standalone; apps read `context.lang` (`get` / `onChange`) and add their strings with `sac.i18n.add(lang, table)`; the host switches with `sac.lang.set()` or a `<sac-lang-toggle>`; `sac.t()` follows the current language and every kit component re-renders its strings on a switch. The kit ships **EN (inline) + DE** (a bundled table) for its own strings; other languages via `sac.i18n.add()`. **Default = the system language** as far as a page can see it: web pages cannot read the OS language, so the first entry of `navigator.languages` the kit has a table for, else EN; a stored choice wins after that.
