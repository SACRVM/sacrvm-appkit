...
- **Every release notifies all consumer repos: signed off 2026-08-23.** A kit release reaches a consumer only when it re-vendors (until the Tier-4 self-contained embed exists, a hosted app runs on the host's kit, so a new version lands only when the repo pulls it). So cutting a release ENDS by sending a re-vendor "update now" message to every consumer app repo — on every release, patch included. The `/release` command carries the step and the current consumer list. (The Tier-4 embed — a CI inliner that ships one self-contained `.js` with `sac-*` tags rewritten to a versioned prefix — is the decided-but-unbuilt endpoint that would remove the need; see the "Tier 4 vision" knowledge doc.)
