...
npx serve .        # http://localhost:3000 — F5 is the whole dev loop`)}
    ...
    ${table(["context", "Description"], [
        ...
        ["<code>host</code>", "The host's package, or <code>null</code> standalone: <code>{ name, icon, href, nav, toolbar }</code>. Your app is COMPLETE — it draws its own <code>&lt;sac-nav&gt;</code>, toolbar and rail (<code>&lt;sac-sidebar&gt;</code> with the <code>items</code> property). The host hands you its presence and you render it: one line, <code>this._nav.host = context.host</code>. Your nav then shows the host's jump-home segment before your brand — same brand recipe, HOST · APP, your segment in accent — the suite's <code>nav</code> entries as a host group in your burger panel, and the host's <code>toolbar</code> controls (a signed-in user, a suite action) at the right end of your ribbon. Optional, your choice: hand your nav your own sections too (<code>this._nav.sections = [{label, href, icon?}]</code>) and they nest indented under your suite entry in the burger — hosted a tree, standalone the flat list. Nothing is ever projected out of the app."],
